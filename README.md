...
Try to commit by feature, not by file count. A good commit should leave the project in a working state and describe one clear change.

Suggested commit split for this project:

1. `chore: add docker setup and environment examples`
2. `feat(auth): add jwt and google authentication flow`
3. `feat(profile): add profile endpoints and frontend screens`
4. `feat(posts): add feed, comments, likes and post creation`
5. `feat(friends): add friend requests and friends list`
6. `feat(files): add upload and protected file access`
7. `feat(chat): add conversations and SignalR chat client`
8. `feat(notifications): add unread count and notification list`
9. `docs: add api notes and project documentation`
10. `refactor: clean up repository structure and shared helpers`

Before every push, run the checks that match the files you changed:

```bash
cd frontend && npm run lint && npm run build
```

For backend changes, build the solution:

```bash
dotnet build backend/Transcendence.sln
```
...
- Do not commit uploaded user files or database backup dumps.
- Keep API changes reflected in the frontend API clients and docs.
- Prefer small pull requests or small commits over one large final commit.
- If a feature touches both backend and frontend, mention both sides in the commit message.

## Current status

The project is still in active development. The core structure is in place, and most of the app areas already have backend endpoints, frontend API clients, and screen-level implementation. The next useful improvements are stronger validation, better error states, fuller tests, and cleaning up development-only files before final submission.
